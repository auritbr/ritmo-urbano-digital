  ...
  return (
    ...
    >
      ...
      <IconCircleButton
        variant="glass-dark"
        size="md"
        className="absolute left-3 top-1/2 -translate-y-1/2 md:left-4"
        onClick={onPrevious}
      ...
      <figure className="max-h-[84dvh] w-full max-w-5xl">
        <img
          src={image.src}
          alt={image.alt}
          className="mx-auto max-h-[76dvh] w-full rounded-lg object-contain"
        />
      ...
      <IconCircleButton
        variant="glass-dark"
        size="md"
        className="absolute right-3 top-1/2 -translate-y-1/2 md:right-4"
        onClick={onNext}
...
export function GalleryButton({
  ...
  return (
    <Button
      variant="ghost"
      className={`group relative h-auto overflow-hidden rounded-lg border border-border bg-card p-0 ${className}`}
      onClick={onClick}
