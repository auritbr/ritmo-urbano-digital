        ...
        <div className="container-site relative">
          <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-3 md:mx-0 md:px-0" aria-label="Selecionar ano da galeria">
            {galleryYears.map((item) => (
               <Button key={item} variant="ghost" className={`liquid-control h-10 shrink-0 rounded-full px-4 ${year === item ? "border-foreground bg-foreground text-background hover:bg-foreground/90" : ""}`} onClick={() => setYear(item)} aria-pressed={year === item}>
                {item}
              </Button>
            ))}
          </div>
