  ...
  return (
    <>
        ...
        <div className="container-site relative flex min-h-[68dvh] items-end pb-10 pt-28 md:min-h-[72dvh] md:pb-14 md:pt-32">
          <div className="max-w-5xl">
            <div className="mb-7 inline-flex items-center gap-3">
              <IconCircleButton asChild variant="glass-dark" size="md" label="Voltar para Notícias"><Link to="/noticias"><ArrowLeft className="transition-transform group-hover:-translate-x-0.5" aria-hidden="true" /></Link></IconCircleButton>
              <Link to="/noticias" className="font-display text-sm font-semibold text-ink-foreground hover:text-brand-primary">Voltar para Notícias</Link>
            </div>
      ...
      <section className="bg-background pb-14 md:pb-16">
        <div className="container-site max-w-[52rem]">
          <h2 className="font-display text-lg font-semibold text-foreground">Compartilhe esta notícia</h2>
          <div className="mt-4 flex flex-wrap gap-3">
            <ShareButton href={`https://wa.me/?text=${encodeURIComponent(shareUrl)}`} icon={WhatsAppIcon} label="Compartilhar no WhatsApp" />
            <ShareButton href={`https://facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} icon={Facebook} label="Compartilhar no Facebook" />
            <ShareButton href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(shareUrl)}`} icon={Linkedin} label="Compartilhar no LinkedIn" />
            <IconCircleButton label="Copiar link" variant="glass-light" size="md" onClick={copyLink}><Copy aria-hidden="true" /></IconCircleButton>
          </div>
        </div>
        {copied ? <div className="fixed bottom-24 left-1/2 z-[90] -translate-x-1/2 rounded-md bg-ink px-4 py-2 text-sm font-semibold text-ink-foreground shadow-soft" role="status">Link copiado!</div> : null}
      </section>
...
function ShareButton({ href, icon: Icon, label }: { href: string; icon: ComponentType<SVGProps<SVGSVGElement>>; label: string }) {
  return <IconCircleButton asChild label={label} variant="glass-light" size="md"><a href={href} target="_blank" rel="noreferrer"><Icon aria-hidden="true" /></a></IconCircleButton>;
}
